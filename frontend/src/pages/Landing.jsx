  ...
  return (
    <div id="top" className="min-h-screen bg-background text-foreground transition-colors duration-200">
      ...
      <section className="relative overflow-hidden border-b border-border/80">
        ...
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[300px] bg-primary/10 dark:bg-primary/15 rounded-full blur-[100px] pointer-events-none" />
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:py-28">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="mt-5 font-display text-4xl font-extrabold leading-[1.08] tracking-tight sm:text-5xl lg:text-6xl text-foreground">
            ...
            <div className="mt-8 flex flex-wrap gap-3.5">
              <Link to="/create" data-testid="hero-create-btn">
                <Button className="h-14 gap-2.5 rounded-xl px-7 text-base font-semibold shadow-md transition-all hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0">
                  <Sparkles className="h-5 w-5" />
                  <span>Buat Prompt Sekarang</span>
                  <ArrowRight className="h-5 w-5" />
                </Button>
              </Link>
              <Button variant="outline" className="h-14 rounded-xl px-7 text-base font-medium border-border/80 hover:bg-secondary/80" onClick={openPricingModal} data-testid="hero-pricing-btn">
            ...
            <div className="mt-8 flex items-center gap-6 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> 100 Token Gratis / Hari
              </span>
              <span className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Reset Otomatis 00:00 WIB
              </span>
              <span className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Siap Salin ke AI Video
              </span>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="absolute -bottom-5 -left-5 hidden max-w-[240px] rounded-xl border border-border bg-card/95 p-4 shadow-xl backdrop-blur-md sm:block">
      ...
      {/* CARA KERJA */}
      <section id="cara-kerja" className="mx-auto max-w-6xl px-5 py-20 sm:px-8">
        <div className="max-w-2xl">
          <span className="text-xs font-bold uppercase tracking-wider text-primary">Langkah Mudah</span>
          <h2 className="mt-1.5 font-display text-3xl font-bold tracking-tight sm:text-4xl">Cara Kerja Sinergi Visual</h2>
          <p className="mt-3 text-muted-foreground">Dari satu foto produk menjadi naskah dan prompt video siap pakai dalam 4 tahap terpadu.</p>
        </div>

        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {STEPS.map((s, i) => (
            <div key={s.title} className="rounded-2xl border border-border/80 bg-card p-6 shadow-sm transition-all hover:border-primary/40 hover:shadow-md hover:-translate-y-1">
              <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary">
                <s.icon className="h-6 w-6" />
              </span>
              <div className="mt-5 text-xs font-bold tracking-wider text-muted-foreground">LANGKAH 0{i + 1}</div>
      ...
      <section className="border-y border-border/80 bg-secondary/30 py-20">
        <div className="mx-auto max-w-6xl px-5 sm:px-8">
          <h2 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">Gaya Video UGC untuk Beragam Kategori</h2>
          <p className="mt-3 max-w-2xl text-muted-foreground">Pilih formula video yang paling sesuai dengan strategi pemasaran produk Anda — AI kami akan mengadaptasikan pacing dan narasinya.</p>
          <div className="mt-8 flex flex-wrap gap-2.5">
            {UGC_STYLES.map((s) => (
              <span key={s} className="rounded-xl border border-border bg-card px-4 py-2.5 text-sm font-semibold shadow-sm transition-colors hover:border-primary/50">
                {s}
              </span>
            ))}
          </div>
      ...
      {/* CONTOH PROMPT */}
      <section id="contoh" className="border-y border-border/80 bg-secondary/30 py-20">
        <div className="mx-auto grid max-w-6xl gap-10 px-5 sm:px-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            ...
            <h2 className="mt-1.5 font-display text-3xl font-bold tracking-tight sm:text-4xl">Contoh Prompt Video UGC</h2>
            <p className="mt-3 text-muted-foreground">Struktur prompt yang komprehensif menjaga konsistensi wajah, pakaian, dan kemasan produk di semua adegan.</p>

            <div className="mt-6 space-y-2.5 rounded-xl border border-border/80 bg-card p-5 text-sm text-muted-foreground shadow-sm">
            ...
            <Link to="/create">
              <Button className="mt-8 h-12 gap-2 rounded-xl px-6 font-semibold">
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-lg">
            <div className="flex items-center justify-between border-b border-border bg-secondary/50 px-5 py-3.5 font-display text-xs font-bold tracking-wider text-foreground">
              <span>OUTPUT PROMPT GOOGLE FLOW</span>
              <span className="text-primary font-mono">100% READY</span>
            </div>
            <pre className="font-mono-prompt max-h-[440px] overflow-auto whitespace-pre-wrap p-5 text-[12px] leading-relaxed text-muted-foreground">
              {SAMPLE_PROMPT}
            </pre>
          </div>
        </div>
      ...
      <section id="paket" className="mx-auto max-w-6xl px-5 py-24 sm:px-8">
        <div className="text-center max-w-2xl mx-auto">
          <span className="text-xs font-bold uppercase tracking-wider text-primary">Sistem Monetisasi & Token</span>
          <h2 className="mt-1.5 font-display text-3xl sm:text-4xl font-extrabold tracking-tight">Paket Langganan & Kuota Harian</h2>
          <p className="mt-3 text-muted-foreground">Mulai gratis dengan kuota 100 token setiap hari atau upgrade ke paket Pro untuk produksi video tanpa batas.</p>
        </div>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-border/80 bg-card p-6 shadow-sm flex flex-col justify-between">
            <div>
              ...
              <div className="mt-4 font-display text-3xl font-extrabold">Rp 0</div>
              <div className="mt-2 text-xs text-primary font-bold flex items-center gap-1.5">
                <Zap className="h-4 w-4 fill-primary" /> 100 Token / Hari (Reset 00:00 WIB)
              </div>
          ...
          {/* Pro */}
          <div className="rounded-2xl border-2 border-primary bg-primary/5 p-6 shadow-xl relative flex flex-col justify-between scale-[1.02]">
            ...
            <div>
              ...
              <div className="mt-4 font-display text-3xl font-extrabold">Rp 99.000 <span className="text-xs font-normal text-muted-foreground">/ bulan</span></div>
              <div className="mt-2 text-xs text-primary font-bold flex items-center gap-1.5">
                <Zap className="h-4 w-4 fill-primary" /> 1.000 Token / Hari (Reset 00:00 WIB)
              </div>
              <ul className="mt-5 space-y-2.5 text-xs text-muted-foreground">
            ...
            <Button className="mt-8 w-full h-11 rounded-xl font-bold shadow-md" onClick={openPricingModal}>Upgrade ke Pro</Button>
          </div>
          ...
          <div className="rounded-2xl border border-border/80 bg-card p-6 shadow-sm flex flex-col justify-between">
            <div>
              ...
              <div className="mt-4 font-display text-3xl font-extrabold">Rp 299.000 <span className="text-xs font-normal text-muted-foreground">/ bulan</span></div>
              <div className="mt-2 text-xs text-primary font-bold flex items-center gap-1.5">
                <Zap className="h-4 w-4 fill-primary" /> 5.000 Token / Hari (Reset 00:00 WIB)
              </div>
              <ul className="mt-5 space-y-2.5 text-xs text-muted-foreground">
            ...
            <Button variant="outline" className="mt-8 w-full h-11 rounded-xl font-bold" onClick={openPricingModal}>Hubungi Sales / Langganan</Button>
          </div>
        </div>
      </section>

      {/* FAQ */}
      <section className="mx-auto max-w-3xl px-5 py-20 sm:px-8 border-t border-border/80">
        <div className="text-center">
          <span className="text-xs font-bold uppercase tracking-wider text-primary">Tanya Jawab</span>
          <h2 className="mt-1.5 font-display text-3xl font-bold tracking-tight sm:text-4xl">Pertanyaan Umum</h2>
        </div>
        <Accordion type="single" collapsible className="mt-8 space-y-2">
          {FAQ.map((f, i) => (
            <AccordionItem key={i} value={`item-${i}`} data-testid={`faq-${i}`} className="border rounded-xl px-4 bg-card">
              <AccordionTrigger className="text-left font-display text-base font-semibold py-4 hover:no-underline">{f.q}</AccordionTrigger>
      ...
      <section className="border-t border-border/80 bg-gradient-to-b from-background to-secondary/30">
        <div className="mx-auto max-w-6xl px-5 py-24 text-center sm:px-8">
          <h2 className="mx-auto max-w-2xl font-display text-3xl font-extrabold tracking-tight sm:text-4xl lg:text-5xl">
          ...
          <Link to="/create" data-testid="cta-create-btn">
            <Button className="mt-8 h-14 gap-2.5 rounded-xl px-9 text-base font-semibold shadow-lg transition-transform hover:-translate-y-0.5">
              <Sparkles className="h-5 w-5" />
      ...
      {/* FOOTER */}
      <footer className="border-t border-border/80 py-10 bg-background">
        <div className="mx-auto max-w-6xl px-5 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-muted-foreground sm:px-8">
